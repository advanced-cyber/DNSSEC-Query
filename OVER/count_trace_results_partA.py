...
if __name__ == "__main__":
    ...
    with open(csv_file) as csv_file:
        ...
        for row in csv_reader:
            ...
            if (website != tmp_website):
                ...
                flag_insecure = True
                index_counter_insecure = 0
                if (status == 'DelegationInsecure'):
                    flag_insecure = False
                    counter_insecure[index_counter_insecure] += 1 
            if(flag_insecure and (status == 'DelegationInsecure' or status == 'NO')):
                flag_insecure = False
                counter_insecure[index_counter_insecure] += 1
    ...
    with open(new_csv_insecure, 'w') as csv_file:
        fieldnames = ['TLD', 'Layot_1', 'Layot_2', 'Layot_3', 'Layot_4',  'Layot_5']
        writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerow({'TLD': counter_insecure[0], 'Layot_1': counter_insecure[1], 'Layot_2': counter_insecure[2], 'Layot_3': counter_insecure[3], 'Layot_4': counter_insecure[4], 'Layot_5': counter_insecure[5]})
    csv_file.close()
